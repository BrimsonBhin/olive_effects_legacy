...
void main()
{
    ...
    fragColor.g = mix(fragColor.r, texture2D(image, vec2(xpos + noise * -0.05, uv.y)).g, 0.25);
    fragColor.b = mix(fragColor.r, texture2D(image, vec2(xpos - noise * 0.05, uv.y)).b, 0.25);

    gl_FragColor = fragColor.rgba;
}